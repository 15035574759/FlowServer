...
class Data extends Base
{
    ...
    public function DatabaseOperation() {
      if(request()->isAjax()){
        $data = input("param.");
        // p($data);
  			if(!isset($data['table_name']) || empty($data['table_name']) || !isset($data['oper']) || empty($data['oper'])) return json(['code' => 0, 'data' => '', 'msg' => '参数有误']);
        $str = strstr($data['table_name'],'_',true);
        // if($str === 'user' || $str === 'dealer') {
          if($data['oper'] == 1) { //删除表
            $res = DB::connect('db_flowuser')->query("DROP TABLE flow_".$data['table_name']);
            if($res == array()) {
              return json(['code' => 1, 'data' => '', 'msg' => '删除表成功']);
            } else {
              return json(['code' => 0, 'data' => '', 'msg' => '删除表失败']);
            }
          } else if($data['oper'] == 2) { //截断表
            $res = DB::connect('db_flowuser')->query("TRUNCATE TABLE flow_".$data['table_name']);
            if($res == array()) {
              return json(['code  ' => 1, 'data' => '', 'msg' => '截断表成功']);
            } else {
              return json(['code' => 0, 'data' => '', 'msg' => '截断表失败']);
            }
          }
        // } else {
        //   return json(['code' => 0, 'data' => '', 'msg' => '您没有权限删除该表']);
        // }
      }
      return $this->fetch('database_operation');
    }
}
