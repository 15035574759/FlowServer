...
class AdModel extends Model
{
    ...
    {
        try{
            $data = DB::name("focus")->order("f_id desc")->select();
    ...
    public function add_focus($param)
    {
        try{
            ...
            $param['img_smail'] = $base->MoveFile("focus",$param['img_smail'])['data'];
    ...
    public function edit_focus($param)
    {
        try{
            ...
            if($param['img_big'] == 1) { //说明图片重新上传了、替换移动图片
                $param['img'] = $base->MoveFile("focus",$param['img'])['data'];
                @unlink(substr($imgurl['img_smail'],1)); //删除原来小图文件
            } else {
                $param['img'] = $param['img'];
            }

            if($param['img_smails'] == 1) { //说明图片重新上传了、替换移动图片
                $param['img_smail'] = $base->MoveFile("focus",$param['img_smail'])['data'];
                @unlink(substr($imgurl['img'],1)); //删除原来文件
            } else {
                $param['img_smail'] = $param['img_smail'];
            }
            // p($param);
            if($param['status'] == 'on'){$param['status'] = 0;}
            $UpdatteArr = [
